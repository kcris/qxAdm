...
struct AutomaticCell : public ICell
{
  ...
  virtual numeric_t getValue() const override
  {
  ...
  virtual QString getDescription() const override
  {
    ...
    return QString("(%1 * %2)")
        .arg(col.getRowInputUnits(m_rowId))
        .arg(col.getUnitPrice());
  }
};
...
QString SplitComponent::getCellDescription(const ICell & cell, const QString & description) const
{
  if (getTitle() == SplitDivided)
  {
    SplitComponent* pSplitCounted = m_ownerCol.getSplitComponent(SplitCounted);
...
{
  numeric_t totalInputsUnits{};

  const InputColumn* pInputCol = nullptr;
...
struct OutputAutoSplitCell : public ICell
{
  ...
  virtual QString getDescription() const
  {
    ...
    foreach(const Column* pComponent, col.components())
    {
      ...
      {
#ifdef NO_DETAILS
        desc.append(QString("%1").arg(pSplit->cellAt(m_rowId)->getValue(pSplit))); //no details
#else
        desc.append(QString("%1").arg(pSplit->cellAt(m_rowId)->getDescription(pSplit))); //details
      }
#endif
    }
...
void OutputAutoSplitColumn::splitAmount()
{
  ...
  Q_ASSERT(compareNumeric(getTotalValue(nullptr/*this*/), getAmount())); //cells sum should match
}
